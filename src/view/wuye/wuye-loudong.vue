  ...
  <div class="menus boxright">
    <div class="filter-container">
        <div class="filter-item">
          <el-button type="primary" @click="handleadd(0,'增加楼栋',true,true)"><i class="el-icon-circle-plus"></i> 增加楼栋</el-button>
        </div>
      </div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row>
      <el-table-column type="expand" align="center">
          <template slot-scope="props">
             <el-table :data="props.row.HouseList" border fit highlight-current-row >
                <el-table-column label="单元" prop="Name" align="center" >
                </el-table-column>         
                <el-table-column label="排序" align="center">
                  <template slot-scope="scope">
                    ...
                    <span @click="sort2(scope.row,scope.$index,+1,props.$index)" :class="scope.$index==props.row.HouseList.length-1?'disabled':''">  
                ...
                <el-table-column label="操作" align="center">
                  <template slot-scope="scope">
                    <el-button
                      ...
                      @click="handleditor(scope.row,'修改单元',false,false)"><i class="el-icon-edit"></i> 编辑</el-button>
                    <el-button
      ...
      <el-table-column label="楼栋名称" prop="Name" align="center">
      </el-table-column>
      <el-table-column label="排序" align="center">
         <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center">
         <template slot-scope="scope">
          <el-button
            size="mini"  type="primary"
            @click="handleditor(scope.row,'修改楼栋',true,false)"><i class="el-icon-edit"></i> 编辑</el-button>
          <el-button
            size="mini" type="danger"
            @click="handledel(scope.row)"><i class="el-icon-delete"></i> 删除</el-button>
          <el-button
            size="mini" type="success"
            @click="handleadd(scope.row,'增加单元',false,true,scope.$index)"><i class="el-icon-circle-plus"></i> 增加单元</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      index:0,//当前index
      rules: {
        Name: [{ required: true, message: '楼栋名称必须填写！', trigger: 'blur' }]
      },
    };
  },
  created() {
  ...
  methods: {
                ...
                this.list.splice(index, 1)
                  ...
                  for(let i in this.list){
                    if(this.list[i].Id===row.ParentId){
                      for (let v in this.list[i].HouseList) {                      
                        if (this.list[i].HouseList[v].Id === row.Id) {
                          this.list[i].HouseList.splice(v, 1)
                          break
                        }
                      }
                    }
            ...
            if(response.Status==1){
              if(this.iscreate){
                ...
                if(this.ismain){
                  ...
                  this.list[this.index].HouseList.push(this.temp);
                }              
              }else{
                if(this.temp.ParentId==0){
                  ...
                  for(let i in this.list){
                    if(this.list[i].Id===this.temp.ParentId){
                      for (let v in this.list[i].HouseList) {                      
                        if (this.list[i].HouseList[v].Id === this.temp.Id) {
                          this.list[i].HouseList[v].Name=this.temp.Name;
                          break
                        }
                      }
                    }
    ...
    sort2: function (item, index, type,indexbig) {//使用方法：传递当前数组的item,index下标,-1为上移，+1为下移   
        this.setup(item, index, type,this.list[indexbig].HouseList);
    },
